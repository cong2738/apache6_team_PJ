...
void nc_draw_gl_npu(struct viewport viewport, int network_task, pp_result_buf *net_result, struct gl_npu_program g_npu_prog)
{
    ...
    /* User Edit */
    static float car_movement_scalar[100] = {0};
    static unsigned int frame_cnt = 0;
    static Point past_pos[100] = {0};
    /* User Edit End */
    ...
    if(network_task == DETECTION)
    {
        ...
        for(int i=0; i<draw_cnn->max_class_cnt; i++){
            for(int bidx = 0; bidx < det_result->class_objs[i].obj_cnt; bidx++) {
                ...
                // draw bbox label
                /* User Edit */
                sprintf(buftext, "[%d] Speed = %0.2f", obj_info.track_id, car_movement_scalar[obj_info.track_id]);
                /* User Edit End*/
